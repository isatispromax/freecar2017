...
void ftm_input_init(FTMn ftmn, FTM_CHn ch, FTM_Input_cfg cfg,uint32 freq_max)
{
    ...
    // ELSnB:ELSnA         1          10          11
    // 配置             上升沿      下降沿      跳变沿
    FTMX[ftmn]->CONTROLS[ch].CnSC &=  ~(FTM_CnSC_ELSA_MASK | FTM_CnSC_ELSB_MASK);
    FTMX[ftmn]->CONTROLS[ch].CnSC |=  cfg << FTM_CnSC_ELSB_SHIFT;
	ps = 0;
...
//  @return     void
//  @since      v1.0
//  Sample usage:               int16 dat = ftm_quad_init(ftm1);	
//-------------------------------------------------------------------------------------------------------------------
