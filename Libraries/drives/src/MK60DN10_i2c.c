...
{ 
  ...
  i2c_write_byte(i2c0,( 0x1e << 1 ) | MWSR);       //发送设备地址+写信号
  i2c_delay();  
  i2c_write_byte(i2c0,0x03);                   //发送存储单元地址，从0x03开始	
  i2c_Start(i2c0);                         //起始信号
  i2c_write_byte(i2c0, ( 0x1e << 1) | MRSW );          //发送设备地址+读信号
  
  i2c_EnterRxMode(i2c0);
  for (i=0; i<6; i++)                      //连续读取6个地址数据，存储中BUF
  {
     buff[i] = I2CN[i2c0]->D;          //BUF[0]存储数据
     if (i == 5)
     {
       i2c_DisableAck(i2c0);                //最后一个数据需要回NOACK
     }
     else
     {
       i2c_Wait(i2c0);                 //回应ACK
