...
void uart_init(UARTn uartn, uint32 baud)
{
    ...
    switch(uartn)
    {
        ...
        case uart5:
        {
            SIM->SCGC1 |= SIM_SCGC1_UART5_MASK;      //使能 UART5 时钟
            if((UART5_RX_PIN == D8) || (UART5_RX_PIN == E9))                                    port_init( UART5_RX_PIN, ALT3);
            if((UART5_TX_PIN == D9) || (UART5_TX_PIN == E8))                                    port_init( UART5_TX_PIN, ALT3);
        }break;
        default:
        return;
    }
