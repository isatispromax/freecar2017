...
uint8 NRF_RF_Addr[NRF_ADDR_WIDTH]={0x66,0x66,0x66,0x66,0x66};   // TX,RX地址,可自行更改
...
void NRF_Send_Packet(uint8 *p)
{
    ...
    NRF_SPI_W_Cmd_Byte(NRF_CMD_FLUSH_TX);
    ...
    //reg = NRF_SPI_R_Reg_Byte(NRF_REG_CONFIG);
     //   systick_delay_ms(30);
...
void NRF_send_data(uint8* p)
{
  ...
  /*ce为低，进入待机模式1*/
   ...
   /*写数据到TX BUF 最大 32个字节*/
  NRF_SPI_W_Reg_Burst(NRF_CMD_W_TX_PAYLOAD, p, NRF_DATA_WIDTH);
...
void NRF_putchar(uint8 ch)
{
  uint8 nrf_putchar_buff[32];
  nrf_putchar_buff[0] = 0x01;
  nrf_putchar_buff[1] = ch;
  NRF_Send_Packet(nrf_putchar_buff); 
}
...
//  @return     void
//-------------------------------------------------------------------------------------------------------------------
void NRF_putbuff (uint8 *buff, uint32 len)
{
    while(len--)
    {
        NRF_putchar(*buff);
...
uint8 NRF_Check(void)
{
	...
	uint8 check_out_buf[5]={0x00},reg1,reg2;

	NRF_SCK(0);
	NRF_CSN(1) ; 
	NRF_CE(0);

	//NRF_SPI_W_Reg_Burst(NRF_CMD_W_REGISTER|NRF_REG_TX_ADDR, check_in_buf, 5); //ox20|0x10         ‭00100000‬   ‭00010000‬
        systick_delay_ms(30);
	NRF_SPI_R_Reg_Burst(NRF_CMD_R_REGISTER|NRF_REG_TX_ADDR, check_out_buf, 5);//0x00|0x10
        systick_delay_ms(30);
        reg1 = NRF_SPI_R_Reg_Byte(NRF_REG_CONFIG);
        systick_delay_ms(30);
        reg2 = NRF_SPI_R_Reg_Byte(NRF_REG_RF_CH);
        systick_delay_ms(30);
